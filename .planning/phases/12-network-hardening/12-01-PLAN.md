...
<task type="auto">
  ...
  <files>N/A — Docker runtime operation, no files modified</files>
  <action>
...
7. External verification: `curl --connect-timeout 5 http://<public-ip>:9200` and `:9201` from VPS (routes via loopback, won't fully simulate external, but iptables -i eth0 rules are the correct defense-in-depth). For true external validation, test from another host or use an online port scanner.
</verification>
